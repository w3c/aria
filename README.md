...
If changes are expected, pull request authors (or reviewers) should check the diff in the action's log. Since that UI is not very accessible, it is helpful to add it as a comment to the pull request discussion. As a pull request author, you may update roleInfo.js as part of the PR but you do not have to.
